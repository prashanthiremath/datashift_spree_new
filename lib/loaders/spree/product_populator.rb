  ...
  module  SpreeEcom
    ...
    class ProductPopulator < Populator
      ...
      def add_taxons
        # TODO smart column ordering to ensure always valid by time we get to associations
        puts "TAXON"
        puts product_load_object.inspect
        puts product_load_object.valid?
        puts product_load_object.errors.full_messages
        product_load_object.save_if_new
        ...
        chain_list.each do |chain|
          ...
          puts "NAME LIST"
          puts parent_name.inspect
          rescue
            puts e.message
          end  
          parent_taxonomy = Spree::Taxonomy.where(:name => parent_name).first_or_create
          puts "PARENT TAXON"
          puts parent_taxonomy.inspect
          ...
          taxons = name_list.collect do |name|
            ...
            begin
              @taxon = Spree::Taxon.where(:name => name, :parent_id => parent.id, :taxonomy_id => parent_taxonomy.id).first_or_create
              puts "TACON IN"
              puts @taxon.inspect
              ...
              unless(@taxon)
                puts ("Missing Taxon - could not find or create #{name} for parent #{parent_taxonomy.inspect}")
              end
            rescue => e
               puts (e.inspect)
               puts "Cannot assign Taxon ['#{taxon}'] to Product ['#{product_load_object.name}']"
              next
            end
          ...
          puts ("Product assigned to Taxons : #{unique_list.collect(&:name).inspect}")

...
module DataShift
  ...
  module SpreeLoading
    ...
    def image_klass
      @image_klass  ||= Spree::Image
    end

    def option_type_klass
      @option_type_klass  ||= Spree::OptionType
    end

    def option_value_klass
      @option_value_klass  ||= Spree::OptionValue
    end

    def property_klass
      @property_klass  ||= Spree::Property
    end

    def product_property_klass
      @product_property_klass  ||= Spree::ProductProperty
    end

    def stock_location_klass
      @stock_location_klass  ||= Spree::StockLocation
    end

    def stock_movement_klass
      @stock_movement_klass  ||= Spree::StockMovement
    end

    def taxonomy_klass
      @taxonomy_klass  ||= Spree::Taxonomy
    end

    def taxon_klass
      @taxon_klass  ||= Spree::Taxon
    end

    def variant_klass
      @variant_klass  ||= Spree::Variant
    end
    ...
    def add_images( record, owner = nil )
      ...
      puts "SPREE IMAGEs"
      owner ||=  Spree::Product.where(id: record.product_id).last
      ...
      value.to_s.split(multi_assoc_delim).each do |image|
        ...
        if(image.match(spree_uri_regexp))
          ...
          puts ("Storing Image in TempFile #{base.inspect}.#{extname.inspect}")
